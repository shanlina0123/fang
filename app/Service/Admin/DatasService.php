...
class DatasService extends AdminBase
{
    ...
    public  function  index()
    {
        ...
        //列表
        $objList=Select::select("id","name","status","cateid","created_at")->orderBy('id','asc')->get();
    ...
    public  function  getOne($cateid)
    {
       ...
       // return Cache::get('datasList', function() {
            ...
            //默认条件
            $list=Select::where("cateid",$cateid)->select("id","name","status","cateid","created_at")->orderBy('id','asc')->get();
            //结果检测
            if(empty($list))
            {
                responseData(\StatusCode::EMPTY_ERROR,"无结果");
